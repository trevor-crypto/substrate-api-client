...
//! Offers macros that build extrinsics for custom runtime pallets based on the metadata.
//! Additionally, some predefined extrinsics for common runtime pallets are implemented.
...
/// Generates the extrinsic's call field for a given pallet and call passed as &str
/// # Arguments
///
...
($node_metadata: expr, $pallet: expr, $call_name: expr $(, $args: expr) *) => {
        {
            let pallet = $node_metadata.pallet_with_calls($pallet).unwrap().to_owned();
...
/// Generates an Unchecked extrinsic for a given pallet and call passed as a &str.
/// # Arguments
///
	...
	$(, $args: expr) *) => {
		{
            ...
            info!("Composing generic extrinsic for pallet {:?} and call {:?}", $pallet, $call);
            let call = $crate::compose_call!($api.metadata.clone(), $pallet, $call $(, ($args)) *);
